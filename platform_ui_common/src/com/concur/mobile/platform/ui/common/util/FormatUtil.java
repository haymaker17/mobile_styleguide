...
/**
 * Ported from core.util, removed dependencies on other core modules:
 * 
 * 1. buildURI() now accepts serverAddress instead of retrieving it from Preferences.getServerAddress()
 * 
 * 2. formatEmployeeName() now accepts boolean isEmail parameter instead of calling Preferences.isTestDriveUser()
 * 
 * @author yiwenw
 * 
 */
public class FormatUtil {
    ...
    public static final DateFormat FULL_WEEKDAY_SHORT_MONTH_DAY_FULL_YEAR;
    ...
    static {
        ...
        FULL_WEEKDAY_SHORT_MONTH_DAY_FULL_YEAR = new SimpleDateFormat("EEEE, MM/dd/yyyy");
    }
     ...
     * @param ssl
     * @param serverAddress
     *            , e.g. acquired via Preferences.getServerAddress()
     * @return
     */
    public static URI buildURI(String path, boolean ssl, String serverAddress) {

        // Build our URI
        StringBuilder serverURI = new StringBuilder(Format.formatServerAddress(ssl, serverAddress)).append(path);
...
/**
     ...
     * @param isEmail
     *            - Preferences.isTestDriveUser()
     * 
     * @return Updated employeeName as single email address if Test Drive user, else EmployeeName
     */
    public static String formatEmployeeName(String employeeName, boolean isEmail) {
        if (isEmail) {
     ...
     * @return
     */
    public static String formatAmountWithNoDecimals(double amount, Locale loc, String crnCode, boolean useSymbol,
            boolean useGrouping) {
        ...
        if (crnCode != null) {
            try {
                ...
                Log.w(Const.LOG_TAG, CLS_TAG + ".formatAmountNoDecimals: invalid currency code: " + crnCode, iae);
            }
        } else {
            Log.e(Const.LOG_TAG, CLS_TAG + ".formatAmountNoDecimals: currency code is null!");
        }
