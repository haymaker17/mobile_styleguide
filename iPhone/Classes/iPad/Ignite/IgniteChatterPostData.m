...
-(void) respondToXMLData:(NSData *)data withMsg:(Msg *)msg// It's JSON, not XML!
{
    ...
    if (error != nil)
    {
        ...
        NSString *errorMessage = [NSString stringWithFormat:@"IgniteChatterPostData::respondToXMLData: Error code = %i, domain = %@, description = %@, failure reason = %@", error.code, errorDomain, localizedDescription, localizedFailureReason];
