...
{//set up the message
	...
	self.path = [NSString stringWithFormat:@"%@/mobile/Home/GetCountSummary",[ExSystem sharedInstance].entitySettings.uri];		
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    else if ([currentElement isEqualToString:@"TravelRequestApprovalCount"]) // Travel Request Trips to Approve 
    {
        dict[@"TravelRequestApprovalCount"] = string;
    }
    else if ([currentElement isEqualToString:@"TripsToApproveCount"]) // New Trips Approvals - TripsToApproveCount
    {
        dict[@"TripsToApproveCount"] = string;
    }
    // Handle Invoices counts 
    else if ([currentElement isEqualToString:@"InvoicesToApproveCount"]) 
    {
        dict[@"InvoicesToApproveCount"] = string;
    }
    else if ([currentElement isEqualToString:@"InvoicesToSubmitCount"]) 
    {
        dict[@"InvoicesToSubmitCount"] = string;
    }
    else if ([currentElement isEqualToString:purchaseRequestsToApproveCount])
    {
        dict[purchaseRequestsToApproveCount] = string;
    }
