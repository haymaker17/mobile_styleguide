...
-(void) viewWillAppear:(BOOL)animated
{
    ...
    if (self.entry.isMergedSmartExpense.boolValue || [ExSystem sharedInstance].isSingleUser) {
...
- (void)viewDidLoad
{
    ...
    // AJC - unused code begin. please delete if unused by 2013-11-29
    //    if([ExSystem sharedInstance].isSingleUser)
    //        self.title = [Localizer getLocalizedText:@"Add Expense"];
    //    else if(self.title == nil)
    //        self.title = [Localizer getLocalizedText:@"Add Expense"];
    // AJC - unused code end. please delete if unused by 2013-11-29
...
- (void)makeFieldsArray:(id)sender
{
    ...
    [self.allFields addObject:[self getExpenseTypeField]];
    [self.allFields addObject:[self getAmountField]];
    [self.allFields addObject:[self getCurrencyField]];
    [self.allFields addObject:[self getDateField]];
    [self.allFields addObject:[self getLocationField]];
    [self.allFields addObject:[self getVendorField]];
    [self.allFields addObject:[self getCommentField]];
    
    self.sectionFieldsMap[@"1"] = self.allFields;
}
...
// AJC - unused code begin. please delete if unused by 2013-11-29
//-(void) configureCell:(FormFieldCell *)cell indexPath:(NSIndexPath *)indexPath
//{
//    FormFieldData* fld = [allFields objectAtIndex:indexPath.row];
//    cell.lblLabel.text = fld.label;
//    cell.lblValue.text = fld.fieldValue;
//
//    [cell.lblErrMsg setHidden:YES];
//
...
//    [self refreshField:fld];//[self fieldUpdated:fld];
//    self.isDirty = YES;
//}
//
...
//{
//    FormFieldData* fld = (FormFieldData*) context;
//	fld.fieldValue = [DateTimeFormatter :date];
...
-(void)showExpenseTypeEditor:(FormFieldData*) field
{
	...
	[self presentModalViewController:expenseTypesVC animated:YES];
}
...
-(void) writeToQueue
{
    ...
    //Send notification for after expense has been queued
    NSNotificationCenter *defaultCenter = [NSNotificationCenter defaultCenter];
    [defaultCenter postNotificationName:@"queue item count changed" object:self];
}
...
{
    // AJC - if this code is still here 2013-10-30, please delete
    /*
     if(entry.updatedImage)
     {
     entry.hasReceipt = @"N";
     ...
     */
    // AJC - end junk code to delete 2013-10-30
...
-(void) saveToMRU
{
    for(FormFieldData *field in self.allFields)
    {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    else if ([self isSaveConfirmDialog:alertView.tag])
	{
		...
		else if (buttonIndex == 2) // No
		{
            ...
            /*
             if (entry.updatedImage)
             {
             entry.updatedImage = NO;
             }
             else if (updatedReceiptImageId)
             {
             updatedReceiptImageId = NO;
             }
             */
    ...
    else if (alertView.tag == kAlertViewRateApp){
        [AppRating processChoiceToRateApp:buttonIndex vc:self alertTag:alertView.tag];
        [self navigateView];
    }
    else
        [self clearActionAfterSave];
...
- (void)showReceiptViewerAndAllowEdits:(BOOL)allowEdits excludeReceiptStoreOption:(BOOL)excludeReceiptStoreOption
{
    ...
    // AJC - unused code 1 line below. please delete if unused by 2013-11-29
    //receiptView.canAppend = allowEdits;
