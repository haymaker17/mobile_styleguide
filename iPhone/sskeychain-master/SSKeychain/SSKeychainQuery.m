...
#endif
    // Touch ID. May need device and OS check6
...
- (BOOL)fetch:(NSError *__autoreleasing *)error {
	__block OSStatus status = SSKeychainErrorBadArguments;
	if (!self.service || !self.account) {
		if (error) {
			*error = [[self class] errorWithCode:status];
		}
    ...
    if ([self.account isEqualToString:@"ACLpassword"] || [self.account isEqualToString:@"ACLuserID"])
    {
        if ([SignInWithTouchID canEvaluatePolicy])
        {
            [query setObject:@NO forKey:(__bridge id)kSecUseNoAuthenticationUI];
            [query setObject:@"Authenticate to login to Concur" forKey:(__bridge id)kSecUseOperationPrompt];
        }
    }
    else
    {
