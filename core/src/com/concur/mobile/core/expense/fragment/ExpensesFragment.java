...
public class ExpensesFragment extends BaseFragment implements INetworkActivityListener {
    ...
    public enum ViewState {
        ...
        LOCAL_DATA_REFRESH,     // Indicates viewing local data with background fetch on-going.
        RESTORE_APP_STATE,      // Indicates the application is restoring state.
        NO_LOCAL_DATA_REFRESH,  // No local data present, server refresh happening.
        NO_DATA                 // No data either locally or from the server.
    };
    ...
    /**
     * Contains a reference to a handler for click events.
     */
    private ViewOnClickHandler viewOnClickHandler;
    ...
    /**
     * Contains a reference to a broadcast receiver for handling data updates.
     */
    private DataUpdateReceiver dataUpdateReceiver;

    /**
     * Contains a reference to an outstanding data update request.
     */
    protected ServiceRequest dataUpdateRequest;

    /**
     * Contains whether or not the data update broadcast receiver is currently registered.
     */
    ...
    private DialogFragment expenseRefreshDialog;
    ...
    public void onAddQuickExpense() {
        ...
        if (expenseTypes != null) {
            Intent intent = new Intent(activity, ExpenseDetail.class);
    ...
    public View buildView(LayoutInflater inflater, Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            DialogFragment newFragment = (ReceiptChoiceDialogFragment) getFragmentManager().findFragmentByTag(
    ...
    private void checkForRefreshData(boolean showNoConnectivityDialog) {
        ...
        if (!expEntCache.hasExpenseList() || expEntCache.shouldRefetchExpenseList()) {
            if (ConcurCore.isConnected()) {
                if (app.getService() != null) {
                    // Check for an outstanding request.
                    if (dataUpdateRequest == null) {
                        // Send a request off to the server to retrieve a list
                        // of mobile entries.
                        // TODO OCR - call new GSEL
                        // dataUpdateRequest = app.getService().sendAllExpenseRequest();
                        if (dataUpdateRequest != null) {
    ...
    public void registerReceivers() {
        ...
        // Register the data update receiver if needbe.
        if (!dataUpdateReceiverRegistered) {
            if (isDataUpdateRequired()) {
                if (dataUpdateReceiver == null) {
                    dataUpdateReceiver = new DataUpdateReceiver();
                }
                activity.registerReceiver(dataUpdateReceiver, getDataUpdateFilter());
    ...
    protected IntentFilter getDataUpdateFilter() {
        return ALL_EXPENSE_FILTER;
    }
    ...
    protected void onDataUpdate(Context context, Intent intent) {
        ...
        if (requestStatus == Const.SERVICE_REQUEST_STATUS_OKAY) {
            ...
            if (httpStatus == HttpStatus.SC_OK) {
                ...
                if (!mwsStatus.equalsIgnoreCase(Const.REPLY_STATUS_SUCCESS)) {
                    ...
                    Log.d(Const.LOG_TAG, CLS_TAG + ".onDataUpdate: MWS error '" + actionStatusErrorMessage + "'.");
                ...
                if (serviceRequest != null && !serviceRequest.isCanceled()) {
                    ...
                    Log.d(Const.LOG_TAG, CLS_TAG + ".onDataUpdate: non HTTP status: '" + lastHttpErrorMessage + "'.");
            ...
            if (serviceRequest != null && !serviceRequest.isCanceled()) {
                Log.d(Const.LOG_TAG,
                        CLS_TAG + ".onDataUpdate: request could not be completed due to: "
    ...
    /*
     * This dialog implementation is a simple carry-over from the original showDialog() implementation. It is far too greatly
     * bound to this activity. The server actions should not be occurring in the dialog. The confirmation should happen then it
     * should return and have the fragment do the actions. Do not replicate the pattern as coded.
     * 
     * TODO: Fix this
     */
    @SuppressLint("ValidFragment")
    private class ConfirmSplitExpensesDialogFragment extends DialogFragment {
            ...
            builder.setPositiveButton(getText(R.string.okay), new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int id) {
                    ...
                    if (smartExpenseToBeSplit != null) {
                        String splitSmartExpenseKey = null;
                        if (smartExpenseToBeSplit.getExpenseEntryType() == Expense.ExpenseEntryType.SMART_CORPORATE) {
                            splitSmartExpenseKey = smartExpenseToBeSplit.getCorporateCardTransaction().getCctKey();
                        } else if (smartExpenseToBeSplit.getExpenseEntryType() == Expense.ExpenseEntryType.SMART_PERSONAL) {
                            splitSmartExpenseKey = smartExpenseToBeSplit.getPersonalCardTransaction().pctKey;
                        } else {
                            Log.e(Const.LOG_TAG,
                                    CLS_TAG
                                            + ".onCreateDialog: non \"smart\" expense type selected for split...selected type is '"
                                            + smartExpenseToBeSplit.getExpenseEntryType().name() + "'.");
                        }
                        if (splitSmartExpenseKey != null) {
                            // if (concurService.addSplitSmartExpenseKey(splitSmartExpenseKey)) {
                            // ConcurCore.getExpenseEntryCache().splitSmartExpense(smartExpenseToBeSplit);
                            // refreshExpenseList();
                            // } else {
                            // Log.e(Const.LOG_TAG,
                            // CLS_TAG
                            // + ".onCreateDialog.onClick(SmartExpenseSplit): failed to add persist split expense key!");
                            // }
                        } else {
                            Log.e(Const.LOG_TAG, CLS_TAG
                                    + ".onCreateDialog.onClick(SmartExpenseSplit): smart expense split key is null!");
                        }
                        smartExpenseToBeSplit = null;
                    } else {
                        Log.e(Const.LOG_TAG, CLS_TAG
                                + ".onCreateDialog.onClick(SmartExpenseSplit): smart expense to be split is 'null'!");
                    }
                    dialog.dismiss();
                }
            });
     ...
     * 
     * @param shouldSmartMatch
     *            Holds whether or not the user selected to use Smart Matching.
     */
    private void getReportList(boolean shouldSmartMatch) {
            ...
            if (listItemAdapter.getItem(((AdapterContextMenuInfo) menuInfo).position) instanceof ExpenseListItem) {
                ...
                menuItem.setOnMenuItemClickListener(new android.view.MenuItem.OnMenuItemClickListener() {
                    ...
                    public boolean onMenuItemClick(android.view.MenuItem item) {
                        smartExpenseToBeSplit = exp;
                        (new ConfirmSplitExpensesDialogFragment()).show(getFragmentManager(), null);
                        return true;
                    }
                });
                menuItem = menu.add(0, Menu.NONE, 0, R.string.edit);
                menuItem.setOnMenuItemClickListener(new android.view.MenuItem.OnMenuItemClickListener() {
                    ...
                    public boolean onMenuItemClick(android.view.MenuItem item) {
                        ...
                        if (exp.getExpenseEntryType() == Expense.ExpenseEntryType.CORPORATE_CARD
                                || exp.getExpenseEntryType() == Expense.ExpenseEntryType.SMART_CORPORATE) {
                            // For corporate/smart corporate expense, pass the
                            // CCT key.
                            CorporateCardTransaction corpCardTrans = exp.getCorporateCardTransaction();
                            intent.putExtra(Const.EXTRA_EXPENSE_CORPORATE_CARD_TRANSACTION_KEY,
                                    corpCardTrans.getCctKey());
                        } else if (exp.getExpenseEntryType() == Expense.ExpenseEntryType.PERSONAL_CARD
                                || exp.getExpenseEntryType() == Expense.ExpenseEntryType.SMART_PERSONAL) {
                            // For personal/smart personal expense, pass the PCT
                            // key.
                        ...
                        } else if (exp.getExpenseEntryType() == Expense.ExpenseEntryType.RECEIPT_CAPTURE) {
                            // For receipt capture pass rc key
                            ReceiptCapture receiptCaptureTrans = exp.getReceiptCapture();
                            intent.putExtra(Const.EXTRA_EXPENSE_RECEIPT_CAPTURE_KEY, receiptCaptureTrans.rcKey);
                        }
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
        ...
        if (itemId == R.id.select_all) {
            ...
            if (ConcurCore.isConnected()) {
                if (dataUpdateRequest == null) {
                    // Send a request to retrieve a new expense list.
                    // TODO OCR - call new GSEL
                    // dataUpdateRequest = app.getService().sendAllExpenseRequest();
                    if (dataUpdateRequest != null) {
                        ViewUtil.setTextViewText(getView(), R.id.loading_data, R.id.data_loading_text,
        ...
        } else if (itemId == R.id.new_ocr_receipt_capture) {
            if (!RolesUtil.isTestDriveUser()) {
                // Create the fragment and show it as a dialog.
                DialogFragment newFragment = new ReceiptChoiceDialogFragment();
                newFragment.show(getFragmentManager(), ReceiptChoiceDialogFragment.DIALOG_FRAGMENT_ID);
            }
        }
        return retVal;
    }
                    ...
                    R.string.general_cancel, new AlertDialogFragment.OnClickListener() {
                        ...
                        public void onClick(FragmentActivity activity, DialogInterface dialog, int which) {
                            ...
                            if (pendingRequest) {
                                ...
                                if (frag instanceof ExpensesFragment) {
                                    DeleteExpenseProgressDialogHandler.show(frag, numOfExpensesToDelete);
                                } else {
                                    Log.e(Const.LOG_TAG, CLS_TAG
                                            + "ExpensesFragment.onDelete getAlertDialog can't reference Expenses!");
                                }
                            }
                        }
    ...
    class AddToReportReceiver extends BroadcastReceiver {
        ...
        final String CLS_TAG = ExpensesFragment.CLS_TAG + "." + AddToReportReceiver.class.getSimpleName();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            AddExpenseProgressDialogHandler.dismiss(ExpensesFragment.this);
            ExpensesFragment.this.addToReportRequest = null;
                        ...
                        if (httpStatusCode == HttpStatus.SC_OK) {
                            if ((intent.getStringExtra(Const.REPLY_STATUS).equalsIgnoreCase(Const.REPLY_STATUS_SUCCESS))
                                ...
                                if (reportKey != null) {
                                    ...
                                    if (ExpensesFragment.this.reportKey == null) {
    ...
    class ReceiptUploadReceiver extends BroadcastReceiver {
        ...
        final String CLS_TAG = ExpensesFragment.CLS_TAG + "." + ReceiptUploadReceiver.class.getSimpleName();
                            ...
                            if (intent.getStringExtra(Const.REPLY_STATUS).equalsIgnoreCase(Const.REPLY_STATUS_SUCCESS)) {
                                ...
                                if (allExpenseDeleteResponsesReceived()) {
                                    ...
                                    if (dataUpdateRequest != null) {
                                        ...
                                        if (view != null) {
                                            ViewUtil.setTextViewText(view, R.id.loading_data, R.id.data_loading_text,
                                                    getText(getDataLoadingTextResourceId()).toString(), true);
                                        }
                                        // Set the view state to indicate data
                                        // being loaded.
                                        viewState = ViewState.LOCAL_DATA_REFRESH;
                                        flipViewForViewState();
                                    }
    ...
    class DeleteExpenseReceiver extends BroadcastReceiver {
        ...
        final String CLS_TAG = ExpensesFragment.CLS_TAG + "." + DeleteExpenseReceiver.class.getSimpleName();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // Dismiss the dialog.
            DeleteExpenseProgressDialogHandler.dismiss(ExpensesFragment.this);
                        ...
                        if (httpStatusCode == HttpStatus.SC_OK) {
                            if (intent.getStringExtra(Const.REPLY_STATUS).equalsIgnoreCase(Const.REPLY_STATUS_SUCCESS)) {
                                ...
                                if (dataUpdateRequest != null) {
                                    ViewUtil.setTextViewText(getView(), R.id.loading_data, R.id.data_loading_text,
                                            getText(getDataLoadingTextResourceId()).toString(), true);
                                    // Set the view state to indicate data being
                                    // loaded.
                                    viewState = ViewState.LOCAL_DATA_REFRESH;
                                    flipViewForViewState();
                                }
    ...
    class ExpenseEntryClickListener implements AdapterView.OnItemClickListener {
        ...
        private final String CLS_TAG = ExpensesFragment.CLS_TAG + "." + ExpenseEntryClickListener.class.getSimpleName();
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (reportListDialogFrag instanceof ActiveReportsListDialogFragment) {
                ...
                if (!activeReportListAdapter.isNewOptionSelected(which)) {
                    ...
                    ExpensesFragment.this.startActivityForResult(intent, Const.CREATE_NEW_REPORT);
                }
            } else {
    ...
    class OnCheckChange implements CompoundButton.OnCheckedChangeListener {
        ...
        private final String CLS_TAG = ExpensesFragment.CLS_TAG + "." + OnCheckChange.class.getSimpleName();
    ...
    static class AddExpenseProgressDialogHandler extends DialogFragmentHandler {
        ...
        public static void show(Fragment frag, int quantity) {
            ...
            message = ConcurCore.getContext().getResources()
        ...
        public void onCancel(FragmentActivity activity, DialogInterface dialog) {
            ...
            if (frag instanceof ExpensesFragment) {
                ExpensesFragment exFrag = (ExpensesFragment) frag;
