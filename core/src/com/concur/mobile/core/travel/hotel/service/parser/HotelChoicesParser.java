...
import com.concur.mobile.core.travel.hotel.data.HotelRoomWithViolation;
...
public class HotelChoicesParser implements Parser {
    ...
    private boolean inHotelBenchmarks;
    private boolean inBenchmarksCollection;
    ...
    public void startTag(String tag) {
        ...
        } else if (tag.equalsIgnoreCase("BenchmarksCollection")) {
            inBenchmarksCollection = true;
        } else if (inBenchmarksCollection) {
            if (tag.equalsIgnoreCase("Benchmarks")) {
                hotelBenchmarks = new ArrayList<HotelBenchmark>();
                inHotelBenchmarks = true;
            } else if (inHotelBenchmarks) {
                if (tag.equalsIgnoreCase("HotelBenchmark")) {
                    hotelBenchmark = new HotelBenchmark();
                }
            }
        } else if (tag.equalsIgnoreCase("TravelPointsBank")) {
            inTravelPointsBank = true;
            travelPointsBank = new TravelPointsBank();
        }
    }
    ...
    public void handleText(String tag, String text) {
        if (tag.equalsIgnoreCase("StartIndex")) {
            ...
            } else if (tag.equalsIgnoreCase("TravelPoints")) {
                cheapestRoom.travelPoints = Parse.safeParseInteger(text);
            } else if (tag.equalsIgnoreCase("CanRedeemTravelPointsAgainstViolations")) {
                cheapestRoom.canRedeemTravelPointsAgainstViolations = Parse.safeParseBoolean(text);
            } else if (tag.equalsIgnoreCase("gdsName")) {
                cheapestRoom.gdsName = text;
            }
        } else if (inCheapestRoomWithViolation) {
            if (tag.equalsIgnoreCase("CrnCode")) {
                ...
                cheapestRoomWithViolation.travelPoints = Parse.safeParseInteger(text);
            } else if (tag.equalsIgnoreCase("CanRedeemTravelPointsAgainstViolations")) {
                cheapestRoomWithViolation.canRedeemTravelPointsAgainstViolations = Parse.safeParseBoolean(text);
            } else if (tag.equalsIgnoreCase("gdsName")) {
                cheapestRoom.gdsName = text;
            }
        ...
        // else if (tag.equalsIgnoreCase("TravelPoints")) {
        // choice.travelPoints = Parse.safeParseInteger(text);
        // } else if (tag.equalsIgnoreCase("CanRedeemTravelPointsAgainstViolations")) {
        // choice.canRedeemTravelPointsAgainstViolations = Parse.safeParseBoolean(text);
        // }
        else if (inBenchmarksCollection) {
            if (inHotelBenchmarks) {
                hotelBenchmark.handleElement(tag, text);
            }
        } else if (inTravelPointsBank) {
            travelPointsBank.handleElement(tag, text);
        } else {
    ...
    public void endTag(String tag) {
        if (inCheapestRoom && tag.equalsIgnoreCase("CheapestRoom")) {
            ...
            if (inHotelBenchmarks) {
                if (tag.equalsIgnoreCase("HotelBenchmark")) {
                    hotelBenchmarks.add(hotelBenchmark);
                } else if (tag.equalsIgnoreCase("Benchmarks")) {
                    inHotelBenchmarks = false;
                }
            } else if (tag.equalsIgnoreCase("BenchmarksCollection")) {
                inBenchmarksCollection = false;
            }
        } else if (inTravelPointsBank) {
