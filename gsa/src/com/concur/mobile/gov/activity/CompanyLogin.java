...
/**
 * An extension of <code>BaseActivity</code> supporting SSO-based corporate
 * login.
 */
public class CompanyLogin extends BaseActivity {

    private static final String CLS_TAG = CompanyLogin.class.getSimpleName();
    ...
    // Contains the receiver used to handle the results of a corporate SSO query.
    ...
    // Contains a reference to an outstanding request to retrieve corp SSO information.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (advanceToCompanyLogon) {
            // Launch the company sign-on activity.
            Intent i = new Intent(this, CompanySignOn.class);
        ...
        getSupportActionBar().setTitle(getString(R.string.sso_sign_in));
            ...
            companyCodeView.setOnEditorActionListener(new OnEditorActionListener() {
                ...
                public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                    if (actionId == EditorInfo.IME_ACTION_GO) {
                        CompanyLogin.this.onClick(loginButton);
                        return true;
                    }
                    return false;
                }
    ...
    /**
     * An extension of <code>BaseBroadcastReceiver</code> for the purposes of handling
     * the results of a corporate SSO query.
     */
    static class CorpSsoQueryReceiver extends BaseBroadcastReceiver<CompanyLogin, CorpSsoQueryRequest> {

        private static final String CLS_TAG = CompanyLogin.CLS_TAG + "." + CorpSsoQueryReceiver.class.getSimpleName();
         ...
         *            the activity.
         */
        CorpSsoQueryReceiver(CompanyLogin activity) {
            super(activity);
        }
         ...
         */
        @Override
        protected void clearActivityServiceRequest(CompanyLogin activity) {
        ...
        protected void handleFailure(Context context, Intent intent) {
            ...
            // ConcurMobile concurMobile = activity.getConcurMobile();
            // CorpSsoQueryReply reply = concurMobile.getCorpSsoQueryReply();
            // if( reply != null ) {
            // if( reply.mwsStatus.equalsIgnoreCase(Const.REPLY_STATUS_NOT_FOUND)) {
            // activity.showDialog(DIALOG_SSO_QUERY_NOT_FOUND);
            // } else {
            // activity.showDialog(DIALOG_SSO_QUERY_FAILED);
            // }
            // } else {
            ...
            if (reply != null) {
                if (reply.ssoEnabled) {
                    if (reply.ssoUrl != null) {
                        // Launch the company sign-on activity.
                        Intent i = new Intent(activity, CompanySignOn.class);
