...
public class Home extends BaseActivity implements View.OnClickListener,
		NavigationListener, ReceiptChoiceListener {
	...
	protected static class RemoteWipeHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			if (liveHome != null && !liveHome.remoteWipe) {
				...
				case 1:
					Log.d(Const.LOG_TAG,
							"RemoteWipeHandler: remote wipe, resetting");
	 ...
	 * ConcurServiceHandler) to indicate that the session has expired and the
	 * user needs to login again. The end result will be a cleared task stack
	 * with the Login activity sitting at the root. See
	...
	public static void expireLogin() {
		...
		((ConcurCore) ConcurCore.getContext()).getService()
	...
	/**
	 * An access method to be used from another thread (presumably the
	 * ConcurServiceHandler) to indicate that the a "remote-wipe" has been sent
	 * down from a login/auto-login response. The end result will be a cleared
	 * task stack with the Login activity sitting at the root. See
	 * ConcurServiceHandler.handleMessage() for the invocation.
	 */
	...
	private final BroadcastReceiver dataReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Const.ACTION_SUMMARY_UPDATED.equals(action)) {
			...
			} else if (Const.ACTION_EXPENSE_REPORT_ENTRY_FORM_UPDATED
					.equals(action)) {
				...
				if (app.getCurrentEntryDetailForm() != null) {
					...
					try {
						...
						Log.w(Const.LOG_TAG, CLS_TAG
								+ ".onReceive: dismissRequestDialog: ", ilaExc);
					}
				}
			} else if (Const.ACTION_EXPENSE_CAR_CONFIGS_UPDATED.equals(action)) {
				decrInProgressRef();
				if (ViewUtil.isShowMileageExpenseOnHomeScreenEnabled(Home.this)
						&& showPersonalCarMileage()) {
				...
				params.put(Flurry.PARAM_NAME_ACTION,
						Flurry.PARAM_VALUE_REFRESH_DATA);
				EventTracker.INSTANCE.track(Flurry.CATEGORY_SETTINGS,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDrawerToggle = new ActionBarDrawerToggle(this, mDrawerLayout,
				R.drawable.concur_logo, R.string.empty_string,
		...
		if (RolesUtil.isTestDriveUser()
				&& Preferences
						.shouldShowTestDriveTips(Const.PREF_TD_SHOW_OVERLAY_HOME)) {
			showTestDriveTips();
		} else {
		...
		if (Preferences.isFirstTimeRunning(prefs)) {
			if (!RolesUtil.isTestDriveUser()
					&& (RolesUtil.isExpenser(Home.this) || RolesUtil
		...
		if (savedInstanceState != null
				&& savedInstanceState
						.containsKey(Const.EXTRA_EXPENSE_IMAGE_FILE_PATH)) {
			receiptCameraImageDataLocalFilePath = savedInstanceState
					.getString(Const.EXTRA_EXPENSE_IMAGE_FILE_PATH);
		}
		if (savedInstanceState != null) {
			upTime = savedInstanceState
					.getLong(Const.ACTIVITY_STATE_UPTIME, 0L);
		}
		...
		boolean isTravelOnly = isTraveler
				&& !(RolesUtil.isExpenser(Home.this)
						|| RolesUtil.isExpenseApprover(Home.this)
						|| RolesUtil.isTRApprover(Home.this)
						|| RolesUtil.isInvoiceApprover(Home.this)
						|| RolesUtil.isInvoiceUser(Home.this) || RolesUtil
		...
		if (isItinViewer) {
			...
			if (Preferences.shouldAllowTravelBooking()
					&& RolesUtil.isTraveler(Home.this)) {
				showBookingRows();
			} else {
				hideBookingUI();
			}
		} else if (Preferences.shouldAllowTravelBooking()
				&& RolesUtil.isTraveler(Home.this)) {
		...
		if (!RolesUtil.isInvoiceApprover(Home.this)
				&& !RolesUtil.isInvoiceUser(Home.this)) {
			hideInvoiceUI();
		}
		...
		if (showTrips) {
			showTripsUI();
			// Show no trips
			getRowSubheader(R.id.homeRowTravel).setText(
					R.string.home_row_travel_open_booking_subheader_negative);
			if (isItinViewer || isTraveler) {
		...
		if (RolesUtil.isExpenser(Home.this)
			...
			dataReceiverFilter
					.addAction(Const.ACTION_EXPENSE_REPORT_ENTRY_FORM_UPDATED);

			// We will receive this when getting the carconfig.
			dataReceiverFilter
					.addAction(Const.ACTION_EXPENSE_CAR_CONFIGS_UPDATED);
		...
		if (!orientationChange) {
			...
			if (itinCache != null) {
				...
				Log.e(Const.LOG_TAG, CLS_TAG
						+ ".onCreate: itinerary cache is null!");
			}
		...
		boolean launch = getIntent()
				.getBooleanExtra(LAUNCH_EXPENSE_LIST, false);
		if (launch) {
			Intent intent;
	...
	private void showRemoteWipeDialog() {
		...
		AlertDialogFragment adf = DialogFragmentFactory.getAlertOkayInstance(
				R.string.account_locked, R.string.account_locked_message);
		...
		adf.show(Home.this.getSupportFragmentManager(),
				"tag.login.remote.wipe.dialog");
	}
	...
	private void showExpiredDialog() {
		...
		b.setPositiveButton(getText(R.string.general_ok),
	...
	private void launchLoginScreen() {
		...
		SessionInfo sessionInfo = ConfigUtil.getSessionInfo(concurMobile
				.getApplicationContext());
		Intent login = null;
		if (((ssoReply != null && ssoReply.ssoEnabled && ssoReply.ssoUrl != null))
				|| (sessionInfo != null && (!TextUtils.isEmpty(sessionInfo
						.getSSOUrl())))) {
			// Company sign-on.
			login = new Intent(Home.this, EmailLookupActivity.class);
			login.putExtra(
					EmailLookupActivity.EXTRA_ADVANCE_TO_COMPANY_SIGN_ON, true);
		} else {
			// PIN-based login.
			login = new Intent(Home.this, EmailLookupActivity.class);
			login.putExtra(
					com.concur.mobile.platform.ui.common.util.Const.EXTRA_LOGOUT,
					true);
		}
		startActivity(login);
	}
	...
	/**
	 * If the user is a test drive user and their account is expired, show an
	 * AlertDialog with our business number attached to it, and upon clicking
	 * "close" on the dialog, log the user out and send them to the login
	 * screen.
	 */
	private void showTestDriveAccountExpiredDialog() {
		...
		final SpannableString dialogBodyText = new SpannableString(
				getString(R.string.test_drive_expiration_message)
						+ Const.TEST_DRIVE_CONTACT_CONCUR_NUMBER);

		final AlertDialog expirationAlertDialog = new AlertDialog.Builder(this)
				.setTitle(R.string.test_drive_expiration_title)
				.setCancelable(false)
				.setMessage(dialogBodyText)
				.setPositiveButton(R.string.test_drive_expiration_close,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int id) {
								...
								// Startup.java.
								Preferences.clearSession(PreferenceManager
										.getDefaultSharedPreferences(Home.this));

								PlatformProperties.setAccessToken(null);
								PlatformProperties.setSessionId(null);

								Intent login = new Intent(Home.this,
		...
		// Linkify link.
		TextView bodyTextView = ((TextView) expirationAlertDialog
	...
	/**
	 * The first time a non Test Drive user logs in, they'll see a tour screen
	 * where they can fling to the next image. These images are selected
	 * dynamically depending on user type (Travel, Expense, Both, or Neither).
	 */
	protected void showTour() {

		/**
		 * A child of SimpleOnGestureListener that overrides fling to allow the
		 * user to flip through tour screens in the ViewFlipper and shows
		 * animation when they do so.
		 */
		class HomeTourGestureDetector extends SimpleOnGestureListener {
			...
			public HomeTourGestureDetector(ViewFlipper flipper,
					ArrayList<View> homeTourDotsList) {
				this.flipper = flipper;
				this.homeTourDotsList = homeTourDotsList;

				Resources res = getResources();
				homeTourTitleStrings = new String[] {
						res.getString(R.string.home_tour_expense1_title),
						res.getString(R.string.home_tour_expense2_title),
						res.getString(R.string.home_tour_travel1_title) };

				homeTourMessageStrings = new String[] {
						res.getString(R.string.home_tour_expense1_message),
			...
			public boolean onFling(MotionEvent e1, MotionEvent e2,
				...
				try {
					...
					if (e1.getX() - e2.getX() > SWIPE_MIN_DISTANCE
						...
						homeTourDotsList.get(currentViewIndex)
								.setBackgroundResource(
										R.drawable.home_tour_blue_dot);
						homeTourTextTitle
								.setText(homeTourTitleStrings[currentViewIndex]);
						homeTourTextMessage
								.setText(homeTourMessageStrings[currentViewIndex]);
		...
		View.OnClickListener dismissListener = new OnClickListener() {
			...
			public void onClick(View v) {
				Preferences.setNotFirstTimeRunning(PreferenceManager
						.getDefaultSharedPreferences(Home.this));
			}
		};

		View homeTourView = UIUtils.setupOverlay((ViewGroup) getWindow()
				.getDecorView(), R.layout.home_tour, dismissListener,
		...
		if (RolesUtil.isExpenser(Home.this)) {
			...
			if (RolesUtil.isTraveler(Home.this)) {
				...
				homeTourDotsList.add(homeTourDots
						.findViewById(R.id.homeTourDot3));
				...
				// for it.
				homeTourDots.findViewById(R.id.homeTourDot3).setVisibility(
						View.GONE);
				homeTourFlipper.removeView(homeTourFlipper
						.findViewById(R.id.homeTourTravel1));
			...
			final GestureDetector gestureDetector = new GestureDetector(this,
					new HomeTourGestureDetector(homeTourFlipper,
							homeTourDotsList));
			...
			homeTourTextTitle.setText(res
					.getString(R.string.home_tour_travel1_title));
			TextView homeTourTextMessage = (TextView) findViewById(R.id.home_tour_text_message);
			homeTourTextMessage.setText(res
					.getString(R.string.home_tour_travel1_message));
	...
	/**
	 * If this user is a test drive user, this will get called the first time
	 * the user loads the home screen to show the test drive tips overlay
	 */
	protected void showTestDriveTips() {
		...
		OnClickListener dismissListener = new OnClickListener() {
			...
			public void onClick(View v) {
				Preferences
						.setShouldNotShowTestDriveTips(Const.PREF_TD_SHOW_OVERLAY_HOME);
				isTipsOverlayVisible = false;

				// Analytics stuff.
				Map<String, String> flurryParams = new HashMap<String, String>();
				upTime = ((System.nanoTime() - startTime) / 1000000000L)
						+ upTime; // Convert nanoseconds to seconds.
				flurryParams.put(Flurry.PARAM_NAME_SECONDS_ON_OVERLAY,
						Flurry.formatDurationEventParam(upTime));
				EventTracker.INSTANCE.track(Flurry.CATEGORY_OVERLAYS,
						"Home Screen", flurryParams);
			}
		};

		UIUtils.setupOverlay((ViewGroup) getWindow().getDecorView(),
				R.layout.td_overlay_home, dismissListener,
	...
	protected void onResume() {
		...
		if (sessionId == null || sessionId.length() == 0) {
			...
			if (getIntent().hasExtra(REMOTE_WIPE)) {
				...
				/*
				 * MOB-15538 requires this check. In this case application has
				 * been forced stopped or killed by system. In that case when
				 * application recreated or restarted this activity also get
				 * created simultaneously. It is quite possible you don't have
				 * session id as db creation has delayed. please see newly
				 * created updateDataBasedOnServiceAvail in onResume and
				 * onServiceAvailable.
				 */
				showExpiredDialog();
			} else {
		...
		Fragment reportListDialogFrag = getSupportFragmentManager()
				.findFragmentByTag(ACTIVE_REPORTS_LIST_TAG);
		if (reportListDialogFrag instanceof ActiveReportsListDialogFragment) {
			...
			Log.w(CLS_TAG,
					"Failed to find fragment for ACTIVE_REPORTS_LIST_TAG");
		}
	...
	/**
	 * if your application targets API level 12 or lower, then your activity
	 * always handles this configuration change via this method. API level 12 or
	 * lower doesnt restart the app if in AndroidManifest we mentioned
	 * android:confiChange="orientation".
	 * */
	@Override
	...
	private void showHideHomeImage() {
		/*
		 * MOB-16309 : Talked to Loc about this JIRA and we have decided to
		 * remove image if the phone is in landscape mode. In future it may
		 * change.
		 */
		...
		case Configuration.ORIENTATION_PORTRAIT:
			...
			cityscape.setImageResource(getResources().getIdentifier(
					Preferences.getCurrentCityscape(), "drawable",
					this.getPackageName()));
			break;
	...
	protected void onServiceAvailable() {
		if (ConcurCore.isConnected()) {
			...
			ConcurException
					.processSavedExceptions((ConcurCore) getApplication());
		}
		...
		if (needService) {
			/*
			 * MOB-15538 : we required restoreReceivers() and
			 * updateOfflineQueueBar() as we are calling from onResume
			 */
			// Restore any receivers.
			restoreReceivers();
	...
	protected void restoreReceivers() {
		...
		if (retainer.contains(ITINERARY_RECEIVER_KEY)) {
			itineraryReceiver = (ItineraryReceiver) retainer
					.get(ITINERARY_RECEIVER_KEY);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		if (receiptCameraImageDataLocalFilePath != null) {
			outState.putString(Const.EXTRA_EXPENSE_IMAGE_FILE_PATH,
	...
	private void showHideOptionMenu(Menu menu) {
		boolean showNotificationbadge = Preferences
	...
	private void logout() {
		...
		if (ConcurMobile.isConnected()) {
			...
			// regardless.
			LogoutRequestTask logoutRequestTask = new LogoutRequestTask(
					getApplicationContext(), REQUEST_ID_LOGOUT,
		...
		if (ssoQueryReply != null && ssoQueryReply.ssoEnabled
				&& ssoQueryReply.ssoUrl != null) {
			i.putExtra(EmailLookupActivity.EXTRA_ADVANCE_TO_COMPANY_SIGN_ON,
					true);
				...
				if (ViewUtil.isTravelProfileComplete(this)
					...
					if (isFromMoreMenu) {
						i.putExtra(Flurry.PARAM_NAME_BOOKED_FROM,
								Flurry.PARAM_VALUE_HOME_MORE);
						isFromMoreMenu = false;
					} else {
						i.putExtra(Flurry.PARAM_NAME_BOOKED_FROM,
								Flurry.PARAM_VALUE_HOME);
					}
					startActivity(i);
				} else {
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		case R.id.homeBook:
			...
			if (isFromMoreMenu) {
				args.putBoolean(BookTravelDialogFragment.IS_FROM_MORE_MENU_ARG,
						isFromMoreMenu);
				isFromMoreMenu = false;
			}
	...
	public void onClick(View v) {
		...
		case R.id.homeBook:
			...
			if (!ConcurCore.isConnected()) {
				 ...
				 * Map<String, String> params = new HashMap<String, String>();
				 * params.put(Flurry.PARAM_NAME_ACTION,
				 * Flurry.PARAM_VALUE_BOOK_TRAVEL);
				 * EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
				 * Flurry.EVENT_NAME_ACTION, params);
				 */
		...
		case R.id.homeRowTravel: {
			...
			i.putExtra(Const.EXTRA_SHOW_TRAVEL_AGENCY_BUTTON,
					showTravelAgencyBtn);
		...
		case R.id.homeRowApprovals: {
			...
			Map<String, String> params = new HashMap<String, String>();
			params.put(Flurry.PARAM_NAME_ACTION,
					Flurry.PARAM_VALUE_VIEW_APPROVALS);
			EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
					Flurry.EVENT_NAME_ACTION, params);
			break;
		}
		case R.id.homeRowExpenses: {
			...
			Map<String, String> params = new HashMap<String, String>();
			params.put(Flurry.PARAM_NAME_ACTION,
					Flurry.PARAM_VALUE_VIEW_EXPENSE_LIST);
			EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
					Flurry.EVENT_NAME_ACTION, params);
			break;
		}
		...
		case R.id.homeQuickExpense:
			...
			if (expenseTypes != null) {
				...
				// cash.
				i.putExtra(Const.EXTRA_EXPENSE_ENTRY_TYPE_KEY,
						Expense.ExpenseEntryType.CASH.name());
				i.putExtra(Flurry.PARAM_NAME_CAME_FROM, Flurry.PARAM_VALUE_HOME);
				i.putExtra(Const.EXTRA_EXPENSE_MOBILE_ENTRY_ACTION,
						Const.CREATE_MOBILE_ENTRY);
				requestCode = Const.CREATE_MOBILE_ENTRY;

				// Flurry Notification
				Map<String, String> params = new HashMap<String, String>();
				params.put(Flurry.PARAM_NAME_ACTION,
						Flurry.PARAM_VALUE_QUICK_EXPENSE);
				EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
						Flurry.EVENT_NAME_ACTION, params);
			} else {
				// Can't go there
		...
		case R.id.homeRowExpenseReports: {
			...
			Map<String, String> params = new HashMap<String, String>();
			params.put(Flurry.PARAM_NAME_ACTION,
					Flurry.PARAM_VALUE_VIEW_REPORTS);
			EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
					Flurry.EVENT_NAME_ACTION, params);
			break;
		}
	...
	private Intent showTravelRequest(Intent i) {
		if (!ConcurCore.isConnected()) {
			...
			i.putExtra(Flurry.PARAM_NAME_CAME_FROM, Flurry.PARAM_VALUE_HOME);
			i.putExtra(RequestListActivity.KEY_SEARCHED_STATUS,
					RequestStatus.ACTIVE.name());

			// Flurry Notification
			Map<String, String> params = new HashMap<String, String>();
			params.put(Flurry.PARAM_NAME_ACTION,
					Flurry.PARAM_VALUE_TRAVEL_REQUEST);
			EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
					Flurry.EVENT_NAME_ACTION, params);
		}
		return i;
	...
	protected void captureReceipt() {
		if (Environment.getExternalStorageState().equals(
			...
			String receiptFilePath = ViewUtil
					.createExternalMediaImageFilePath();
			...
			Log.d(Const.LOG_TAG, CLS_TAG
					+ ".captureReceipt: receipt image path -> '"
					+ receiptCameraImageDataLocalFilePath + "'.");
	...
	protected Dialog onCreateDialog(int id) {
		...
		if (dlg == null) {
			...
			case Const.DIALOG_EXPENSE_NO_MILEAGE_FORM: {
				...
				dlgBldr.setPositiveButton(getText(R.string.okay),
						new DialogInterface.OnClickListener() {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case Const.CREATE_NEW_REPORT: {
			...
			if (data != null && data.hasExtra(Const.EXTRA_EXPENSE_REPORT_KEY)) {
				String reportKey = data
						.getStringExtra(Const.EXTRA_EXPENSE_REPORT_KEY);
				if (reportKey != null) {
		...
		case Const.CREATE_MILEAGE_EXPENSE:
			if (resultCode == Activity.RESULT_OK) {
				...
				i.putExtra(Const.EXTRA_EXPENSE_REPORT_KEY,
						data.getStringExtra(Const.EXTRA_EXPENSE_REPORT_KEY));
				i.putExtra(Const.EXTRA_EXPENSE_REPORT_SOURCE, data.getIntExtra(
		...
		case Const.REQUEST_CODE_BOOK_HOTEL:
			if (resultCode == Activity.RESULT_OK) {
				SharedPreferences prefs = PreferenceManager
						.getDefaultSharedPreferences(ConcurMobile.getContext());
		...
		case REQUEST_TAKE_PICTURE: {
			if (resultCode == Activity.RESULT_OK) {
				if (copyCapturedImage()) {
					...
					Map<String, String> params = new HashMap<String, String>();
					params.put(Flurry.PARAM_NAME_ACTION,
							Flurry.PARAM_VALUE_CAMERA);
					params.put(Flurry.PARAM_NAME_RESULT,
							Flurry.PARAM_VALUE_OKAY);
					...
					it.putExtra(Const.EXTRA_EXPENSE_RECEIPT_URL_KEY,
							strBldr.toString());
					it.putExtra(Const.EXTRA_RECEIPT_ONLY_FRAGMENT, true);
					it.putExtra(Const.EXTRA_SHOW_MENU, true);
					it.putExtra(Const.EXTRA_EXPENSE_IMAGE_FILE_PATH,
							receiptImageDataLocalFilePath);
					it.putExtra(Flurry.PARAM_NAME_FROM,
							Flurry.PARAM_VALUE_CAMERA);
					it.putExtra(Const.EXTRA_EXPENSE_SCREEN_TITLE_KEY,
							getText(R.string.receipt_image));
					startActivity(it);
					...
					EventTracker.INSTANCE.track(Flurry.CATEGORY_RECEIPTS,
							Flurry.EVENT_NAME_FAILURE, params);
	...
	private boolean copyCapturedImage() {
		...
		retVal = ImageUtil
				.compressAndRotateImage(receiptImageDataLocalFilePath);
		if (!retVal) {
	...
	protected void incrInProgressRef() {
		if (++inProgressRef > 0) {
			if (refreshActionItem != null && refreshProgressView != null) {
				// Toggle the progress indicator
				MenuItemCompat.setActionView(refreshActionItem,
						refreshProgressView);
			}
		}
	}
	...
	protected void loadData() {
		...
		if (RolesUtil.isExpenser(Home.this)
			...
			if (ConcurMobile.isConnected()) {
				countSummaryRequest = concurMobile.getService()
						.sendCountSummaryRequest(true);
				...
				if (RolesUtil.isExpenser(Home.this)) {
					carConfigsRequest = concurMobile.getService()
							.sendCarConfigsRequest();
				}
		...
		if (RolesUtil.isTraveler(Home.this)
				|| RolesUtil.isItinViewer(Home.this)
				|| RolesUtil.isTRApprover(Home.this)
				|| RolesUtil.isInvoiceApprover(Home.this)
				|| RolesUtil.isInvoiceUser(Home.this)
			...
			if (ConcurMobile.isConnected()) {
				itinerarySummaryListRequest = concurMobile.getService()
						.sendItinerarySummaryListRequest(true);
	...
	/**
	 * Will examine the result of an attempt to retrieve summary information and
	 * update the UI.
	 * 
	 * @param intent
	...
	protected void updateExpenseSummaryInfo(Intent intent) {
		...
		int serviceRequestStatus = intent.getIntExtra(
				Const.SERVICE_REQUEST_STATUS, -1);
		if (serviceRequestStatus != -1) {
			if (serviceRequestStatus == Const.SERVICE_REQUEST_STATUS_OKAY) {
				int httpStatusCode = intent.getIntExtra(
						Const.REPLY_HTTP_STATUS_CODE, -1);
				if (httpStatusCode != -1) {
					if (httpStatusCode == HttpStatus.SC_OK) {
						if (!(intent.getStringExtra(Const.REPLY_STATUS)
								.equalsIgnoreCase(Const.REPLY_STATUS_SUCCESS))) {
							actionStatusErrorMessage = intent
									.getStringExtra(Const.REPLY_ERROR_MESSAGE);
							Log.e(Const.LOG_TAG,
									CLS_TAG
											+ ".onReceive: mobile web service error -- "
											+ actionStatusErrorMessage + ".");
						}
					} else {
						lastHttpErrorMessage = intent
								.getStringExtra(Const.REPLY_HTTP_STATUS_TEXT);
						Log.e(Const.LOG_TAG, CLS_TAG
								+ ".onReceive: http error -- "
								+ lastHttpErrorMessage + ".");
					}
				} else {
					Log.e(Const.LOG_TAG, CLS_TAG
							+ ".onReceive: missing http reply code!");
				}
			} else {
				if (countSummaryRequest != null
						&& !countSummaryRequest.isCanceled()) {
					Log.e(Const.LOG_TAG,
							CLS_TAG
									+ ".onReceive: service request error -- "
									+ intent.getStringExtra(Const.SERVICE_REQUEST_STATUS_TEXT));
				}
			}
		} else {
			Log.e(Const.LOG_TAG, CLS_TAG
					+ ".onReceive: missing service request status!");
		}
		...
		if (summary != null) {
			...
			if (RolesUtil.isExpenser(Home.this)) {
				...
				if (unsubCount > 0) {
					...
					txtvExpensesReportBadge.setText(Integer
							.toString(unsubCount));
				} else {
			...
			if (RolesUtil.isTravelApprover(Home.this)
				...
				if (RolesUtil.isInvoiceApprover(Home.this)
						|| RolesUtil.isInvoiceUser(Home.this)) {
					approvalCount += summary.invoicesToApprove
							+ summary.invoicesToSubmit;
				}
			...
			int expenseCount = summary.corpCardTransCount
					+ summary.mobileEntryCount + summary.persCardTransCount
	...
	/**
	 * Determines whether personal car mileage should be displayed on action
	 * menu. Requires car configs to have been loaded.
	 * 
	 */
	private boolean showPersonalCarMileage() {
		...
		SharedPreferences prefs = PreferenceManager
				.getDefaultSharedPreferences(this);
		String userCurrencyCode = prefs.getString(Const.PREF_USER_CRN_CODE,
				null);
		...
		if (carConfigList != null && carConfigList.size() > 0) {
			if (RolesUtil.isExpenser(Home.this)) {
				for (int i = 0; i < carConfigList.size(); i++) {
					...
					if (carConfig != null) {
						if (carConfig.crnCode
								.equalsIgnoreCase(userCurrencyCode)
								&& ((carConfig.configType
										.equalsIgnoreCase(CarConfig.TYPE_PER_ONE) && carConfig.rates
										.size() > 0))
								...
								|| ((carConfig.configType
										.equalsIgnoreCase(CarConfig.TYPE_PER_VAR) && carConfig.details
										.size() > 0))) {
							return true;
						}
					}
	...
	protected void updateTripInfo(Intent intent) {
		...
		if (serviceRequestStatus != -1) {
			if (serviceRequestStatus == Const.SERVICE_REQUEST_STATUS_OKAY) {
				...
				if (itinerarySummaryListRequest != null
						&& !itinerarySummaryListRequest.isCanceled()) {
					Log.e(Const.LOG_TAG,
							CLS_TAG
									+ ".onReceive: service request error -- "
									+ intent.getStringExtra(Const.SERVICE_REQUEST_STATUS_TEXT));
	...
	private void updateTripUI() {
		...
		if (RolesUtil.isTraveler(Home.this)
			...
			if (itinCache != null
					&& itinCache.getItinerarySummaryListUpdateTime() != null) {
				...
				if (trips != null && trips.size() > 0) {
					...
					Calendar curTime = Calendar.getInstance(TimeZone
							.getTimeZone("UTC"));
					...
					while (tripIter.hasNext()) {
						...
						} else if (trip.endLocal != null
								&& trip.endLocal.after(curTime)) {
							// MOB-11072 Try using the local time.
							if (trip.startLocal != null
									&& trip.startLocal.before(curTime)) {
								// Active trip
								activeTrips.add(trip);
							} else {
								// Must be in the future. Add it to upcoming.
					...
					if (activeTripCount > 0 || upcomingTripCount > 0) {
						...
						txtvTripBadge.setText(Integer
								.toString(upcomingTripCount + activeTripCount));
					} else {
	...
	/**
	 * Sets the text size of the footer buttons to a smaller size. This is
	 * called if all four footer buttons are showing because some locales have
	 * long translations and cause text to wrap in the default size.
	 */
	private void setSmallFooterButtonText() {
		float textSize = getResources().getDimension(
				R.dimen.homeFooterSmallText);
	...
	/**
	 * An implementation of <code>DialogInterface.OnClickListener</code> to
	 * handle selecting a report.
	 */
	class SelectReportDialogClickListener implements
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			Map<String, String> params = new HashMap<String, String>();
			params.put(Flurry.PARAM_NAME_ACTION,
					Flurry.PARAM_VALUE_PERSONAL_CAR_MILEAGE);
			EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
					Flurry.EVENT_NAME_ACTION, params);
			...
			if (reportListDialogFrag instanceof ActiveReportsListDialogFragment) {
				...
				if (!activeReportListAdapter.isNewOptionSelected(which)) {
					...
					// process.
					ExpenseReport report = (ExpenseReport) activeReportListAdapter
							.getItem(which);

					ConcurCore ConcurCore = (ConcurCore) getApplication();
					IExpenseReportCache expRepCache = ConcurCore
							.getExpenseApprovalCache();
					if (!expRepCache.hasReportDetail(report.reportKey)) {
						...
						// Just get it now.
						ConcurService concurService = ConcurCore.getService();
						concurService.sendReportDetailRequest(report.reportKey,
								Const.EXPENSE_REPORT_SOURCE_ACTIVE);
					}
					...
					intent.putExtra(Const.EXTRA_EXPENSE_REPORT_SOURCE,
							Const.EXPENSE_REPORT_SOURCE_NEW);
					Home.this.startActivityForResult(intent,
							Const.CREATE_NEW_REPORT);
				}
			} else {
				Log.w(CLS_TAG,
						"Failed to find fragment for ACTIVE_REPORTS_LIST_TAG");
	...
	protected void getMileageEntryForm(String reportKey) {
		...
		service.sendReportEntryFormRequest(Const.EXPENSE_TYPE_MILEAGE,
				reportKey, null);
	...
	private void requestLastKnownLocation() {
		LastLocationTracker locTracker = ((ConcurCore) getApplication())
				.getLocationTracker();
		locTracker.startLocationTrace(CLS_TAG, null, true,
				Const.LOC_UPDATE_MIN_TIME, Const.LOC_UPDATE_MIN_DISTANCE);
	}
	...
	private void sendItineraryRequest(String itinLocator) {
		...
		if (itineraryRequest == null) {
			Log.e(Const.LOG_TAG,
					CLS_TAG
							+ ".sendItineraryRequest: unable to create itinerary request.");
	...
	private void registerItineraryReceiver() {
		if (itineraryReceiver == null) {
			...
			getApplicationContext().registerReceiver(itineraryReceiver,
					itineraryFilter);
		} else {
			Log.e(Const.LOG_TAG,
					CLS_TAG
							+ ".registerItineraryReceiver: itineraryReceiver is *not* null!");
		}
	}
	...
	private void unregisterItineraryReceiver() {
		if (itineraryReceiver != null) {
			try {
				...
				Log.e(Const.LOG_TAG, CLS_TAG
						+ ".unregisterItinerarySReceiver: illegal argument",
						ilaExc);
			}
			...
			Log.e(Const.LOG_TAG,
					CLS_TAG
							+ ".unregisterItineraryReceiver: itineraryReceiver is null!");
		}
	}
	 ...
	 */
	static class ItineraryReceiver extends
			BaseBroadcastReceiver<Home, ItineraryRequest> {
		 ...
		 * 
		 * @see
		 * com.concur.mobile.corp.activity.BaseActivity.BaseBroadcastReceiver#
		 * clearActivityServiceRequest(com.concur.mobile.corp.activity
		 * .BaseActivity)
		 */
		@Override
		...
		protected void handleSuccess(Context context, Intent intent) {
			if (intent.hasExtra(Const.EXTRA_ITIN_LOCATOR)) {
				String itinLocator = intent
						.getStringExtra(Const.EXTRA_ITIN_LOCATOR);
				if (itinLocator != null) {
					...
					Map<String, String> params = new HashMap<String, String>();
					params.put(Flurry.PARAM_NAME_ACTION,
							Flurry.PARAM_VALUE_VIEW_CURRENT_TRIP);
					EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
							Flurry.EVENT_NAME_ACTION, params);
				} else {
					Log.e(Const.LOG_TAG, CLS_TAG
							+ ".handleSuccess: itin locator has invalid value!");
				}
			} else {
				Log.e(Const.LOG_TAG, CLS_TAG
						+ ".handleSuccess: itin locator missing!");
			}
		}
		 ...
		 * @see
		 * com.concur.mobile.corp.activity.BaseActivity.BaseBroadcastReceiver#
		 * setActivityServiceRequest(com.concur.mobile.corp.activity.
		 * BaseActivity, com.concur.mobile.corp.service.ServiceRequest)
		 */
		@Override
	 ...
	 * @return
	 */
	protected AsyncTask<Void, Void, Integer> initImageManipulator(
			final ImageSwitcher imgSwitcher) {
		Log.d(Const.LOG_TAG, "imgMan // init");
		return new AsyncTask<Void, Void, Integer>() {

			int[] cityscapes = { R.drawable.city_01, R.drawable.city_02,
					R.drawable.city_03, R.drawable.city_04, R.drawable.city_05,
					R.drawable.city_06, R.drawable.city_07, R.drawable.city_08,
					R.drawable.city_09 };
			...
			protected Integer doInBackground(Void... params) {
				...
				Log.d(Const.LOG_TAG, "imgMan // doIB // cityscape = "
						+ cityscape);
				...
				if (cityscape != null) {
					...
					if (Preferences.shouldUpdateCityscape()) {
						...
						Log.d(Const.LOG_TAG, "imgMan // resId = "
								+ cityscapeResourceId);
					}
				}
	...
	/**
	 * Initialize all of the Navigation buttons to be runnable inside of the
	...
	private void initNavigationMenu() {
		...
		if (!isProfileDisable) {
			...
			setSegNavItem = new DefaultTextNavigationItem(NAVIGATION_HEADER,
					R.layout.navigation_segment,
					R.string.home_navigation_settings, false);
			navItems.add(setSegNavItem);

			// Profile access.
			navItem = new HomeScreenSimpleNavigationItem(NAVIGATION_PROFILE,
					-1, R.string.home_navigation_profile,
					R.drawable.profile_icon_name, View.VISIBLE, View.VISIBLE,
					new Runnable() {
		...
		// Add the navigation segment bar.
		DefaultTextNavigationItem setSegNavItemProfile = new DefaultTextNavigationItem(
				NAVIGATION_HEADER, R.layout.navigation_segment,
				R.string.home_navigation_settings, false);
		...
		if (Preferences.shouldAllowTravelBooking()
				&& RolesUtil.isTraveler(Home.this)) {
			navItem = new HomeScreenSimpleNavigationItem(
					NAVIGATION_BOOK_TRAVEL, -1,
					R.string.home_navigation_book_travel,
					R.drawable.icon_menu_book, View.VISIBLE, View.VISIBLE,
					new Runnable() {
						...
						public void run() {
							...
							if (homeBookTravel != null) {
								...
								Map<String, String> params = new HashMap<String, String>();
								params.put(Flurry.PARAM_NAME_BOOKED_FROM,
										Flurry.PARAM_VALUE_BOOK_TRAVEL);
								EventTracker.INSTANCE.track(
										Flurry.CATEGORY_HOME_MORE,
										Flurry.EVENT_NAME_ACTION, params);
							}
						}
					});
		...
		if (RolesUtil.showPriceToBeatGenerator(Home.this)) {
			navItem = new HomeScreenSimpleNavigationItem(
					NAVIGATION_TRAVEL_POINTS, -1,
					R.string.home_navigation_price_to_beat,
					R.drawable.icon_menu_book, View.VISIBLE, View.VISIBLE,
					new Runnable() {
		...
		if (!ViewUtil.isReceiptStoreHidden(this)
				&& RolesUtil.isExpenser(Home.this)) {
			navItem = new HomeScreenSimpleNavigationItem(NAVIGATION_RECEIPTS,
					-1, R.string.receipts, R.drawable.icon_menu_receipt,
					View.VISIBLE, View.VISIBLE, new Runnable() {
						...
						public void run() {
							...
							intent.putExtra(Const.EXTRA_RECEIPT_ONLY_FRAGMENT,
									true);
							startActivity(intent);
							...
							Map<String, String> params = new HashMap<String, String>();
							params.put(Flurry.PARAM_NAME_ACTION,
									Flurry.PARAM_VALUE_RECEIPT_STORE);
							EventTracker.INSTANCE.track(Flurry.CATEGORY_HOME,
									Flurry.EVENT_NAME_ACTION, params);
						}
					});
		...
		if (RolesUtil.isExpenser(Home.this)
				&& ViewUtil.isShowMileageExpenseOnHomeScreenEnabled(Home.this)) {
			// Check whether or not we need to show the mileage icon in the
			// navigation drawer.
			int navMileageVisibility = (showPersonalCarMileage()) ? View.VISIBLE
					: View.GONE;
			navItem = new HomeScreenSimpleNavigationItem(
					NAVIGATION_PERSONAL_CAR_MILEAGE, -1,
					R.string.home_navigation_car_mileage,
					R.drawable.icon_menu_mileage, View.VISIBLE,
		...
		// Location Check-in.
		if (ViewUtil.isLNAUser(getBaseContext())
				&& RolesUtil.canCheckInLocation(Home.this)) {
			navItem = new HomeScreenSimpleNavigationItem(NAVIGATION_CHECK_IN,
					-1, R.string.home_action_location_check_in,
					R.drawable.icon_menu_location, View.VISIBLE, View.VISIBLE,
					new Runnable() {
						...
						public void run() {
							if (!ConcurMobile.isConnected()) {
								...
								Map<String, String> params = new HashMap<String, String>();
								params.put(Flurry.PARAM_NAME_ACTION,
										Flurry.PARAM_VALUE_LOCATION_CHECK_IN);
								EventTracker.INSTANCE.track(
										Flurry.CATEGORY_HOME,
										Flurry.EVENT_NAME_ACTION, params);
							}
						}
					});
		...
		if (RolesUtil.isTraveler(Home.this) && RolesUtil.isExpenser(Home.this)) {
			navItem = new HomeScreenSimpleNavigationItem(NAVIGATION_LEARN_MORE,
					-1, R.string.home_navigation_learn_more,
					R.drawable.icon_menu_help, View.VISIBLE, View.VISIBLE,
					new Runnable() {
		...
		if (Preferences.shouldShowTripItAd()) {
			navItem = new HomeScreenSimpleNavigationItem(
					NAVIGATION_APP_TRIP_IT, -1,
					R.string.home_navigation_tripit,
					R.drawable.icon_menu_tripit, View.VISIBLE, View.VISIBLE,
					new Runnable() {

						public void run() {
							// First check for the paid version (package
							// name: "com.tripit.paid").
							Intent i = ViewUtil.getPackageLaunchIntent(
									Home.this, "com.tripit.paid");
							if (i == null) {
								// Second, try for the free version (package
								// name: "com.tripit").
								i = ViewUtil.getPackageLaunchIntent(Home.this,
										"com.tripit");
							}
							if (i == null) {
							...
							if (i != null) {
								...
								Map<String, String> params = new HashMap<String, String>();
								params.put(Flurry.PARAM_NAME_ACTION,
										Flurry.PARAM_VALUE_TRIP_IT);
								EventTracker.INSTANCE.track(
										Flurry.CATEGORY_EXTERNAL_APP,
										Flurry.EVENT_NAME_LAUNCH, params);
							}
						}
					});
			navItems.add(navItem);
		...
		if (Preferences.isOCRUser()) {
			navItem = new HomeScreenSimpleNavigationItem(
					NAVIGATION_APP_EXPENSE_IT, -1,
					R.string.home_navigation_expenseit,
					R.drawable.icon_menu_expenseit, View.VISIBLE, View.VISIBLE,
					new Runnable() {

						public void run() {
							Intent i = ViewUtil.getPackageLaunchIntent(
									Home.this, "com.expenseit");
							if (i == null) {
		 ...
		 * 
		 * navItem = new
		 * HomeScreenSimpleNavigationItem(NAVIGATION_APP_TRAVEL_TEXT, -1,
		 * R.string.home_navigation_traveltext, R.drawable.icon_travel_text,
		 * View.VISIBLE, View.VISIBLE, new Runnable() {
		 * 
		 * public void run() { Intent i = null; String url =
		 * "http://www.traveltext.net/CMobile"; i = new
		 ...
		 * EventTracker.INSTANCE.track(Flurry.CATEGORY_EXTERNAL_APP,
		 * Flurry.EVENT_NAME_LAUNCH, params);
		 * 
		 * } } }); navItems.add(navItem);
		 */
		...
		/*
		 * Separator no longer wanted, per
		 * https://concur.aha.io/features/CM-252.
		 * 
		 * if (navItems.size() > 1) { // Add the apps navigation segment bar.
		 * DefaultTextNavigationItem segNavItem = new
		 * DefaultTextNavigationItem(NAVIGATION_HEADER,
		 ...
		 * Removed per https://concur.aha.io/features/CM-252
		 * 
		 * if (isTaxiUser()) { navItem = new
		 * HomeScreenSimpleNavigationItem(NAVIGATION_APP_TAXI_MAGIC, -1,
		 * R.string.home_navigation_curb, R.drawable.icon_menu_curb,
		 * View.VISIBLE, View.VISIBLE, new Runnable() {
		 * 
		 * public void run() { Intent i =
		 * ViewUtil.getTaxiMagicIntent(Home.this); if (i == null) { String url =
		 * "http://taximagic.com"; i = new Intent(Intent.ACTION_VIEW);
		 * i.setData(Uri.parse(url)); } if (i != null) { startActivity(i); //
		 * Flurry Notification Map<String, String> params = new HashMap<String,
		 * String>(); params.put(Flurry.PARAM_NAME_ACTION,
		 * Flurry.PARAM_VALUE_CURB);
		 * EventTracker.INSTANCE.track(Flurry.CATEGORY_EXTERNAL_APP,
		 ...
		 * Removed per https://concur.aha.io/features/CM-252
		 * 
		 * navItem = new HomeScreenSimpleNavigationItem(NAVIGATION_APP_UBER, -1,
		 * R.string.home_navigation_uber, R.drawable.icon_menu_uber,
		 * View.VISIBLE, View.VISIBLE, new Runnable() {
		 * 
		 * public void run() { Intent i =
		 * ViewUtil.getPackageLaunchIntent(Home.this, "com.ubercab"); if (i ==
		 * null) { // app not available hence launch the mobile web site i = new
		 * Intent(Intent.ACTION_VIEW);
		 * i.setData(Uri.parse("https://m.uber.com")); } if (i != null) {
		 * startActivity(i); // Flurry Notification Map<String, String> params =
		 * new HashMap<String, String>(); params.put(Flurry.PARAM_NAME_ACTION,
		 * Flurry.PARAM_VALUE_UBER);
		 * EventTracker.INSTANCE.track(Flurry.CATEGORY_EXTERNAL_APP,
		 * Flurry.EVENT_NAME_LAUNCH, params); } } }); navItems.add(navItem);
		 */
		...
		addNavigationItems(itemContainer, adContainer, inflater, navItems,
				adNavItem);
	}
	...
	protected void showMileageDialog() {
		...
		if (!ConcurMobile.isConnected()) {
			...
			if (!Preferences.shouldAllowReports()) {
				...
				activeReportListAdapter = new ActiveReportsListAdapter(
						Home.this, true, false);
	...
	/**
	 * Goes through all of the items to be added to the Navigation Drawer and
	 * adds them one at a time.
	 * 
	 * @param itemContainer
	 *            The LinearLayout inside of the Navigation Drawer that holds
	 *            all of the NavigationItems
	 * 
	 * @param adContainer
	 *            The LinearLayout below itemContainer in the NavigationDrawer
	 *            that holds the Ad Item
	 * 
	 * @param inflater
	 *            The LayoutInflater
	 * 
	 * @param navItems
	 *            The actual list of Navigation Items that we're to add to the
	 *            Drawer
	 * 
	 * @param adNavItem
	 ...
	 * 
	 */
	protected void addNavigationItems(ViewGroup itemContainer,
			ViewGroup adContainer, LayoutInflater inflater,
		...
		if (navItems != null) {
			...
			for (int i = 0; i < NavItemsSize; i++) {
				...
				if ((navItem instanceof TextNavigationItem)
						|| (navItem instanceof SimpleNavigationItem)) {
					...
					int txtResId = -1;
					if (navItem instanceof TextNavigationItem) {
						txtResId = ((TextNavigationItem) navItem)
								.getTextResId();
							...
							try {
								if (navItem.getId() != NAVIGATION_HEADER) {
									navView.findViewById(
											R.id.navigation_item_separator)
					...
					if (txtResId != -1) {
						TextView txtView = (TextView) navView
					...
					if (navItem instanceof SimpleNavigationItem) {
						iconVisibility = ((SimpleNavigationItem) navItem)
								.getIconVisibility();
						if (iconVisibility == View.VISIBLE) {
							iconResId = ((SimpleNavigationItem) navItem)
									.getIconResourceId();
							ImageView imgView = (ImageView) navView
							...
							ViewUtil.setVisibility(navView, R.id.icon,
									View.GONE);
						}

						// layout visibility
						int layoutVisibility = ((SimpleNavigationItem) navItem)
					...
					View customView = ((CustomNavigationItem) navItem)
							.getView();
					if (customView != null) {
						...
						if (navItem.isSelectable()) {
							customView
									.setOnClickListener(new NavigationItemOnClickListener());
						}
	...
	/**
	 * The OnClickListener for items inside of the Navigation Drawer. Basically
	 * just check which item was clicked and hand off to the
	 * {@link #onItemSelected(navItem) onItemSelected} method to handle which
	 * item was pressed.
	 * 
	 */
		...
		public void onClick(View view) {
			if (view.getTag() != null) {
				try {
					...
					Log.e(Const.LOG_TAG,
							CLS_TAG
									+ ".onClick: view tag object not instance of 'NavigationItem'!");
				}
			} else {
	 ...
	 * onItemSelected(com.concur.core.fragment.navigation.Navigation
	 * .NavigationItem)
	 */
	...
	/**
	 * If this is a first-time run with a notification-enabled build, prompt the
	 * user to see if they wish to allow notifications.
	 */
	 ...
	 * ReceiptChoiceListener#onCameraSuccess(java.lang.String)
	 */
	 ...
	 * 
	 * @see com.concur.mobile.core.dialog.ReceiptChoiceDialogFragment.
	 * ReceiptChoiceListener#onCameraFailure(java.lang.String)
	 */
	public void onCameraFailure(String filePath) {
		DialogFragmentFactory.getAlertOkayInstance(
				this.getText(
						R.string.dlg_expense_camera_image_import_failed_title)
						.toString(),
				R.string.dlg_expense_camera_image_import_failed_message).show(
				getSupportFragmentManager(), null);
	}
	 ...
	 * 
	 * @see com.concur.mobile.core.dialog.ReceiptChoiceDialogFragment.
	 * ReceiptChoiceListener#onGallerySuccess(java.lang.String)
	 */
	 ...
	 * 
	 * @see com.concur.mobile.core.dialog.ReceiptChoiceDialogFragment.
	 * ReceiptChoiceListener#onGalleryFailure(java.lang.String)
	 */
	...
	public void onStorageMountFailure(String filePath) {
		DialogFragmentFactory
				.getAlertOkayInstance(
						this.getText(
								R.string.dlg_expense_no_external_storage_available_title)
								.toString(),
	...
	/**
	 * If the user is using a device running an Android version below 4.1, pop
	 * up a notification and vibrate to inform them that their device will no
	 * longer be receiving Concur app updates until they get on Android 4.1 or
	 * higher.
	 */
	private void showMinSDKIncreaseMessage() {
		...
		TextView linkToMessageCenter = (TextView) minSdkUpgradeMessage
				.findViewById(R.id.minSdkTextLinkToMessageCenter);
		linkToMessageCenter.setPaintFlags(linkToMessageCenter.getPaintFlags()
		...
		linkToMessageCenter.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				findViewById(R.id.minSdkUpgradeMessage)
		...
		View cancelButton = minSdkUpgradeMessage
				.findViewById(R.id.minSdkMessageClose);
...
class HomeScreenSimpleNavigationItem extends DefaultSimpleNavigationItem {
	...
	HomeScreenSimpleNavigationItem(int id, int layoutResId, int textResId,
			int iconResId, int iconVisibility, int navItemVisibility,
			Runnable run) {
		super(id, layoutResId, textResId, iconResId, iconVisibility,
				navItemVisibility);
		this.run = run;
	}
